package com.excellent.controller.student;

import com.excellent.model.Course;
...
public class StudentController {
    ...
    public String courseDetail(@Param("classId") int classId,Model model,HttpServletRequest request){
        if(courseService.checkStuIns(classId,(int)request.getSession().getAttribute("stuid"))){
            ...
            model.addAttribute("msg","选课失败！请注意课程的学院限制");
    ...
    public String searchListByTeaId(@Param("teaid") int teaid,Model model,HttpServletRequest request){
        Integer stuid=(int)request.getSession().getAttribute("stuid");
        List<Course> cor_list=courseService.queryAllByteaId(teaid,stuid);
        model.addAttribute("paging",pageService.subList(1,cor_list));
        model.addAttribute("teaList",userService.queryAllTeacher());
        model.addAttribute("insList",courseService.queryAllIns());
        return "student/courseList";
    ...
    public String searchListByInsId(@Param("insid") int insid,Model model,HttpServletRequest request){
        Integer stuid=(int)request.getSession().getAttribute("stuid");
